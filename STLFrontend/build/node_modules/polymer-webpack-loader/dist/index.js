...
var _htmlLoader2 = require('html-loader');

var _htmlLoader3 = _interopRequireDefault(_htmlLoader2);
...
var ProcessHtml = function () {
  ...
  _createClass(ProcessHtml, [{
    ...
    value: function links(_links) {
      ...
      var shouldRewrite = void 0;
      if (this.options.ignorePathReWrite) {
        shouldRewrite = (0, _RuleSet.normalizeCondition)({ not: this.options.ignorePathReWrite });
      } else {
    ...
    value: function styles(_styles) {
      ...
      _styles.forEach(function (styleElement) {
        var styleContent = (0, _dom.getTextContent)(styleElement);
        ...
        var postcssPipeline = (0, _postcss2.default)([ProcessHtml.postcssParserPlugin(parserCssOptions)]);
        var options = {
    ...
    value: function isExternalPath(node, attributeName) {
      ...
      var parseLink = _url2.default.parse(path, false, true);
    ...
    value: function htmlLoader(node, options) {
      ...
      var serializedSource = _parse2.default.serialize(parseObject);
      var minifiedSource = _htmlLoader3.default.call({
        options: {
          htmlLoader: options
        }
      }, serializedSource);
      ...
      return function (css) {
        function processNode(node) {
          ...
          switch (item.type) {
            ...
            case 'url':
              if (item.url.replace(/\s/g, '').length && !/^#/.test(item.url) && _loaderUtils2.default.isUrlRequest(item.url, parserOptions.root)) {
        ...
        css.walkDecls(function (decl) {
          var localDecl = decl;
          var values = _cssSelectorTokenizer2.default.parseValues(decl.value);
          values.nodes.forEach(function (value) {
            value.nodes.forEach(processNode);
          });
          localDecl.value = _cssSelectorTokenizer2.default.stringifyValues(values);
        });
        ...
        css.walkAtRules(function (rule) {
          ...
          var localRule = rule;
          var values = _cssSelectorTokenizer2.default.parseValues(rule.params);
          values.nodes.forEach(function (value) {
            value.nodes.forEach(processNode);
          });
          localRule.params = _cssSelectorTokenizer2.default.stringifyValues(values);
        });
      };
    }
...
ProcessHtml.postcssParserPlugin = _postcss2.default.plugin('polymer-webpack-loader-parser', ProcessHtml.postcssPlugin);
